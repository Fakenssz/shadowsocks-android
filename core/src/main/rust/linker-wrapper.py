...
if (sys.platform == "msys"
        or sys.platform == "cygwin") and len("".join(args)) > 8191:
    import codecs
    import tempfile

    def posix2win(path):
        CCP_POSIX_TO_WIN_W = 1
        size = cygwin.cygwin_conv_path(CCP_POSIX_TO_WIN_W,
                                       str(path).encode(), 0, 0)
        retval = ctypes.create_unicode_buffer(size)
        cygwin.cygwin_conv_path(CCP_POSIX_TO_WIN_W,
                                str(path).encode(), retval, size)
        return retval.value
...
if code == 0:
    ...
    if sys.platform == "msys" or sys.platform == "cygwin" or sys.platform == "win32":
        linkargs = list(
            filter(
                lambda s: s.startswith("@") and s.find("linker-arguments") !=
                -1,
                sys.argv[1:],
            ))
        if linkargs != []:
    ...
    shutil.copyfile(sys_argv[sys_argv.index("-o") + 1],
                    os.environ["RUST_ANDROID_GRADLE_TARGET"])
if linkargfileName != "":
